...
const PengajarScreen = () => {
   ...
   useEffect(() => {
      fetch("/api/user/students").then(res => {
         return res.json();
      }).then(dataJson => {
   ...
   return (
      <div className="flex py-5 px-6">
            ...
            <form onSubmit={submitHandle} className="p-2">
               <div className="flex flex-col px-3 py-2">
                  <div className="mb-2 rounded-md">
                     <input type="text" name="title" placeholder="Judul" value={titleFile} onChange={e => setTitleFile(e.target.value)} className="p-3 w-full rounded-md" />
                  </div>
                  <div className="mb-2 rounded-md">
                     <textarea rows="5" cols="45" placeholder="Deskripsi" value={descFile} onChange={e => setDescFile(e.target.value)} className="mb-0 p-3 rounded-md "></textarea>
                  </div>
                  <div className="rounded-md p-[0.5px]">
                     <select className="w-full p-2 rounded-md" onChange={(e) => setCategoryFile(e.target.value)}>
                  ...
                  <div className="mt-2">
                     <input type="file" ref={fileRef} className="block w-full text-sm text-slate-800
                  ...
                  <button type="submit" className="mt-8 w-40 p-2 mx-auto rounded-full text-white bg-emerald-700 hover:bg-emerald-800 active:outline-2 active:bg-emerald-900 uppercase">Enter</button>
               </div>
            </form>
         ...
         <div className="daftar-student rounded-md overflow-hidden transition ease-in-out delay-100 shadow-lg hover:shadow-2xl">
            <h3 className='my-2 text-black text-center text-third pt-2 pb-1 w-2/3 mx-auto'>Daftar Pelajar</h3>
            ...
            <div className='px-3 py-2 mx-10  h-40 overflow-hidden hover:overflow-auto border-dashed border-l-2 border-emerald-700 rounded-lg'>
               {dataStudent.map(data => {
                  return (
                     <div key={data._id} className='flex relative rounded-lg my-3 overflow-hidden shadow-xl text-third'>
                        <div className='mr-10 ml-3 my-2'>
                           <p className='pr-5'>{data.email}</p>
                           <p className='pr-5'>{data.namaDepan} {data.namaBelakang}</p>
                        </div>
                        <button type="button" className='absolute right-2 top-5 p-1 rounded-full shadow-lg hover:bg-black hover:text-white' onClick={() => submitDetailStudent(data._id)}>{<AiOutlineCloudDownload />}</button>
                     </div>
                  )
               })}
            </div>
            ...
            <div className='Total-pelajar flex mt-3 p-3 justify-evenly'>
               <div className='p-2 rounded-md shadow-md'>
                  ...
                  <p className=' w-10 text-center mx-auto mt-1 text-zinc-800 text-3xl text-third'>{dataStudent.length}</p>
               </div>
               <div className='p-2 shadow-md rounded-md text-center'>
                  <h3 className='px-2 text-third'>Pesan</h3>
                  <button type="button" onClick={() => { }} className='p-2 mt-1 rounded-xl mx-auto text-2xl hover:bg-emerald-600 active:bg-emerald-700 hover:text-white transition delay-100 ease-in-out outline-2 focus:outline-none focus:ring focus:ring-emerald-800'>{<BsChatLeftText />}</button>
               </div>
